...
class ImageDetectionsField(RawField):
    def __init__(self, preprocessing=None, postprocessing=None, train_detections_path=None, valid_detections_path=None, max_detections=100,
                 sort_by_prob=False, load_in_tmp=True, train_json_path=None, valid_json_path=None, features=False):
        self.max_detections = max_detections
        self.train_detections_path = train_detections_path
        self.sort_by_prob = sort_by_prob
        
        self.features_path = {
            'train': train_detections_path,
            'valid': valid_detections_path
        }

        train_json_data = json.load(open(train_json_path, 'r'))['images']
        valid_json_data = json.load(open(valid_json_path, 'r'))['images']
        self.train_mapping_img2idx = {sample['file_name']: sample['id'] for sample in train_json_data}
        self.valid_mapping_img2idx = {sample['file_name']: sample['id'] for sample in valid_json_data}
    ...
    def preprocess(self, x, avoid_precomp=False):
        ...
        try:
            image_id = self.train_mapping_img2idx[x]
            self.split = 'train'
        except:
            image_id = self.valid_mapping_img2idx[x]
            self.split = 'valid'

        if self.features:

            f = h5py.File(self.features_path[self.split], 'r')
